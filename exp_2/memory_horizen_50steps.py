...
# --- CONFIGURATION ---
# THE FIX: Densely sample the critical 0-500 transition window
STEPS_LIST = [0, 50, 100, 200, 300, 400, 500, 1000, 2000, 3000, 4000, 5000] 
...
print("Starting High-Resolution Memory Horizon Experiment...")
...
# Dynamic Annotations
idx_50 = STEPS_LIST.index(50)
idx_500 = STEPS_LIST.index(500)
plt.text(STEPS_LIST[idx_50], results_mse[idx_50] + 0.005, 'Memory Start', color='purple')
plt.text(STEPS_LIST[idx_500], results_mse[idx_500] - 0.005, 'Memory Collapse Boundary', color='darkred')
if len(results_mse) > 4:
    plt.text(STEPS_LIST[-1], results_mse[-1] - 0.005, 'Liquid Chaos', color='gray', ha='right')

plt.title('The Memory Horizon: Decay of Geometric Information', fontsize=14)
...
plt.savefig('memory_horizon_dense.png')
print("Saved 'memory_horizon_dense.png'.")

# Save Data
df = pd.DataFrame({'Steps': STEPS_LIST, 'MSE': results_mse})
df.to_csv('memory_horizon_dense_results.csv', index=False)
print(df)